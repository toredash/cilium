...
get_cluster_snat_map_v4(__u8 cluster_id __maybe_unused)
{
#if defined(ENABLE_CLUSTER_AWARE_ADDRESSING) && defined(ENABLE_INTER_CLUSTER_SNAT)
	if (cluster_id != 0 && cluster_id != CLUSTER_ID) {
		__u32 idx = cluster_id;

		return map_lookup_elem(&PER_CLUSTER_SNAT_MAPPING_IPV4, &idx);
	}
#endif
...
static __always_inline void *
get_cluster_snat_map_v6(__u8 cluster_id __maybe_unused)
{
#if defined(ENABLE_CLUSTER_AWARE_ADDRESSING) && defined(ENABLE_INTER_CLUSTER_SNAT)
	if (cluster_id != 0 && cluster_id != CLUSTER_ID) {
		__u32 idx = cluster_id;

		return map_lookup_elem(&PER_CLUSTER_SNAT_MAPPING_IPV6, &idx);
	}
#endif
