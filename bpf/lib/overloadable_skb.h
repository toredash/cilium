 ...
 */
static __always_inline __maybe_unused void
set_cluster_id_mark(struct __sk_buff *ctx, __u8 cluster_id)
{
	ctx->mark |= (__u32)cluster_id | MARK_MAGIC_CLUSTER_ID;
}

static __always_inline __u8 ctx_get_cluster_id_from_mark(struct __sk_buff *ctx)
{
	__u8 ret = 0;
